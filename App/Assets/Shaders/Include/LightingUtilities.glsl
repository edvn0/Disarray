...
vec3 calculate_directional_light_textured(DirectionalLight light, vec3 normal, vec3 view_direction, float shadow, uint spec_pow, vec4 albedo, vec4 specular);

vec3 calculate_point_light(
vec4 position, vec4 factors, vec4 ambient, vec4 diffuse, vec4 specular, vec3 normal, vec3 fragment_position, float shadow, vec3 view_direction);
vec3 calculate_spot_light(
vec4 position, vec4 factors, vec4 ambient, vec4 diffuse, vec4 specular, vec3 normal, vec3 fragment_position, float shadow, vec3 view_direction);
vec3 calculate_point_light(
vec4 position, vec4 factors, vec4 ambient, vec4 diffuse, vec4 specular, vec3 normal, vec3 fragment_position, float shadow, vec3 view_direction, vec4 albedo, vec4 specular);
vec3 calculate_spot_light(
vec4 position, vec4 factors, vec4 ambient, vec4 diffuse, vec4 specular, vec3 normal, vec3 fragment_position, float shadow, vec3 view_direction, vec4 albedo, vec4 specular);
...
vec3 calculate_directional_light_textured(DirectionalLight light, vec3 normal, vec3 view_direction, float shadow, uint spec_pow, vec4 albedo_texture, vec4 specular_texture)
{
    const vec3 light_direction = normalize(-light.direction);
    // diffuse shading
    const float diff = max(dot(normal, light_direction), 0.0);
    ...
    const float spec = pow(max(dot(view_direction, reflection_direction), 0.0), spec_pow);
    // combine results
    vec3 ambient = light.ambient.xyz * albedo_texture.xyz;
    vec3 diffuse = light.diffuse * diff * albedo_texture.xyz;
    vec3 specular = light.specular * spec * specular_texture.xyz;
    const float inverse_shadow_factor = 1.0F - shadow;
    return (ambient + inverse_shadow_factor * (diffuse + specular));
}

vec3 calculate_point_light(vec4 position, vec4 input_factors, vec4 input_ambient, vec4 input_diffuse, vec4 input_specular, vec3 normal,
...
vec3 calculate_point_light(vec4 position, vec4 input_factors, vec4 input_ambient, vec4 input_diffuse, vec4 input_specular, vec3 normal,
vec3 fragment_position, float shadow, vec3 view_direction, vec4 diffuse_texture, vec4 specular_texture)
{
    const vec3 light_to_frag_vector = vec3(position) - fragment_position;
    const vec3 light_direction = normalize(light_to_frag_vector);
    // diffuse shading
    ...
    // combine results
    vec4 ambient = input_ambient * diffuse_texture;
    vec4 diffuse = input_diffuse * diff * specular_texture;
    vec4 specular = input_specular * spec * specular_texture;
    ambient *= attenuation;
    diffuse *= attenuation;
    specular *= attenuation;
    const float inverse_shadow_factor = 1 - shadow;
...
vec3 calculate_spot_light(vec4 position, vec4 input_factors, vec4 input_ambient, vec4 input_diffuse, vec4 input_specular, vec3 normal,
vec3 fragment_position, float shadow, vec3 view_direction, vec3 spot_light_direction, float spot_light_cutoff, float spot_light_outer_cutoff, vec4 diffuse, vec4 specular)
{
    const vec3 light_to_frag_vector = vec3(position) - fragment_position;
    const vec3 light_direction = normalize(light_to_frag_vector);
    const float theta = dot(light_direction, spot_light_direction);
    ...
    vec3 colour = calculate_point_light(
    position, input_factors, input_ambient, input_diffuse, input_specular, normal, fragment_position, shadow, view_direction, diffuse, specular);
    return intensity * colour;
}
...
{
    return calculate_spot_light(position, input_factors, input_ambient, input_diffuse, input_specular, normal, fragment_position, shadow,
