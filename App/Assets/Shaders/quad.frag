...
vec3 calc_point_light(PointLight light, float shadow, vec3 normal, vec3 fragPos, vec3 view_direction) {
    return calculate_point_light(light.position, light.factors, light.ambient, light.diffuse, light.specular, normal, fragPos, shadow, view_direction);
}
...
void main() {
    ...
    colour = pc.colour * vec4(out_vec, 0.0f);
