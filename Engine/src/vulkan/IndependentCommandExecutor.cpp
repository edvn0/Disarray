...
IndependentCommandExecutor::~IndependentCommandExecutor() { destroy_executor(); }

void IndependentCommandExecutor::recreate_executor(bool should_clean)
{
	if (should_clean) {
		destroy_executor();
	}
...
void IndependentCommandExecutor::create_query_pools()
{
...
void IndependentCommandExecutor::destroy_executor()
{
	const auto& vk_device = supply_cast<Vulkan::Device>(device);
...
void IndependentCommandExecutor::create_base_structures()
{
	const auto vk_device = supply_cast<Vulkan::Device>(device);
	...
	for (auto& buffer : command_buffers) {
		DebugMarker::set_object_tag(vk_device, buffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, 0, sizeof(demo2), &demo2);
...
void IndependentCommandExecutor::wait_indefinite() { vkDeviceWaitIdle(supply_cast<Vulkan::Device>(device)); }

void IndependentCommandExecutor::begin()
{
...
void IndependentCommandExecutor::begin(VkCommandBufferBeginInfo begin_info)
{
	active = command_buffers[buffer_index()];
	verify(vkBeginCommandBuffer(active, &begin_info));
...
void IndependentCommandExecutor::end()
{
	if (props.record_stats) {
...
void IndependentCommandExecutor::submit_and_end()
{
	end();
...
void IndependentCommandExecutor::record_stats()
{
	if (props.record_stats) {
		// Timestamp query
