...
template <> struct fmt::formatter<const glm::vec2> : fmt::formatter<std::string_view> {
	auto format(const glm::vec2& format, format_context& ctx) -> decltype(ctx.out());
};

template <> struct fmt::formatter<const glm::vec3> : fmt::formatter<std::string_view> {
	auto format(const glm::vec3& format, format_context& ctx) -> decltype(ctx.out());
};

template <> struct fmt::formatter<const glm::vec4> : fmt::formatter<std::string_view> {
	auto format(const glm::vec4& format, format_context& ctx) -> decltype(ctx.out());
};

template <> struct fmt::formatter<const glm::quat> : fmt::formatter<std::string_view> {
